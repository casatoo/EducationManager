			...
			<div>
				<form action="../article/list?">
					<input type="hidden" name="boardId" value="${param.boardId}" /> <select
						class="input-article-select rounded-lg" data-value="${param.searchFrom}" name="searchFrom" required>
					...
					</select> <input class="input-article-search rounded-lg" type="text" placeholder="검색어를 입력해주세요" name="searchWord"
						value="${param.searchWord}" />
					<div class="article-list-search-btn"><button type="submit" >검색</button></div>
				</form>
			</div>
		</div>
		...
		<div class="flex justify-center mb-11">
			<div class="mt-4">
				...
				<c:if test="${startPage > 1}">
					<button class="page-move-btn" onclick="location.href='../article/list${pageBaseUri}&page=1';">1</button>
				</c:if>
				<c:if test="${startPage > 2}">
					<button class="page-move-btn">...</button>
				</c:if>
				<c:forEach var="pageNum" begin="${startPage}" end="${endPage}">
					<button class="page-move-btn ${page == pageNum ? 'page-move-btn' : '' }" onclick="location.href='../article/list${pageBaseUri}&page=${pageNum}';">${pageNum}</button>
				</c:forEach>
				<c:if test="${endPage < pageCount-1}">
					<button class="page-move-btn">...</button>
				</c:if>
				<c:if test="${endPage < pageCount}">
					<button class="page-move-btn" onclick="location.href='../article/list${pageBaseUri}&page=${pageCount}';">${pageCount}</button>
				</c:if>
			</div>
		</div>
	</div>
